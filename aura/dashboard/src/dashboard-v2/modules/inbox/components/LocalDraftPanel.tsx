...
export function LocalDraftPanel({
  ...
  const hasTemplates = authoring.templates.length > 0;
  const templateHelperText = hasTemplates
    ? `${authoring.templates.length} saved ${authoring.templates.length === 1 ? 'template' : 'templates'} available for this browser-local draft.`
    : 'No saved templates are available in Settings.';
  const signatureHelperText = authoring.hasSignature
    ? 'Saved signature is available for this browser-local draft.'
    : 'No saved signature is available in Settings.';

  return (
    <DashboardV2Surface className="v2-inbox-draft-panel" tone="elevated" data-testid="v2-inbox-local-draft">
      <div className="v2-inbox-draft-panel__sidebar">
      ...
      <div className="v2-inbox-draft-panel__composer">
        <div className="v2-inbox-draft-panel__toolbar">
          <div className="v2-inbox-draft-panel__template-control">
            <DashboardV2Select
              ...
              placeholder={hasTemplates ? 'Select a template' : 'No saved templates in Settings'}
              selectedKey={selectedTemplateId || null}
              onSelectionChange={onTemplateChange}
              isDisabled={!hasTemplates}
            />
          </div>
          <div className="v2-inbox-draft-panel__helper-strip" aria-label="Local draft helper status">
            <DashboardV2Text className="v2-inbox-draft-panel__helper-note" tone="muted">
              {templateHelperText}
            </DashboardV2Text>
            <DashboardV2Button
              tone="secondary"
              ...
              isDisabled={!hasTemplates || !selectedTemplateId}
            >
              Insert template
            </DashboardV2Button>
            <DashboardV2Button
